...
class CycleParams:
    ...
    # provozní bod výparníku/kondenzátoru. IHX varianta navíc používá
    # vnitřní výměník tepla řízený parametrem dT_IHX.
...
@dataclass
class EvalStats:
    # Diagnostika evaluací. requests zahrnuje všechny požadavky na fitness,
    # misses odpovídá drahým voláním REFPROP/CoolProp.
    requests: int = 0
    cache_hits: int = 0
    cache_misses: int = 0
    ...
    def add(self, other: "EvalStats") -> None:
        ...
        self.actual_evaluations += other.actual_evaluations
...
def fractions_to_units(fractions: Sequence[float], fraction_step: float) -> Tuple[int, ...]:
    """Převede podíly na celočíselné dílky, např. 0.02 -> 1 jednotka."""
    fractions = normalize_fractions(fractions)
    units = np.rint(fractions / fraction_step).astype(np.int64)
    total_units = fraction_units(fraction_step)

    if np.any(units <= 0):
        raise ValueError("Kazda aktivni slozka musi mit alespon jeden dil podilu.")
    if int(np.sum(units)) != total_units:
        raise ValueError(
            "Podily nelezi presne na diskretni mrizce. "
            "Pouzij quantize_fractions pred vytvorenim klice."
        )
    if not np.allclose(units.astype(np.float64) * fraction_step, fractions):
        raise ValueError("Podily nejsou nasobkem zvoleneho fraction_step.")

    return tuple(int(unit) for unit in units)


def canonicalize_mixture(
    fluids: Sequence[str], fractions: Sequence[float], fraction_step: float
) -> Tuple[Tuple[str, ...], np.ndarray, Tuple[int, ...]]:
    """Vrátí kanonický tvar směsi nezávislý na pořadí složek.

    Fyzikálně stejné směsi:
      A=20 %, B=30 %, C=50 %
      C=50 %, A=20 %, B=30 %
    skončí jako totožný klíč, protože se nejdřív vytvoří dvojice
    (látka, celočíselný podíl) a ty se seřadí podle názvu látky.
    """
    if len(fluids) != len(fractions):
        raise ValueError("Pocet latek a podilu se neshoduje.")
    if len(set(fluids)) != len(fluids):
        raise ValueError("Smes obsahuje duplicitni latky.")

    units = fractions_to_units(fractions, fraction_step)
    pairs = sorted(zip((str(fluid) for fluid in fluids), units), key=lambda item: item[0])
    canonical_fluids = tuple(fluid for fluid, _ in pairs)
    canonical_units = tuple(unit for _, unit in pairs)
    canonical_fractions = units_to_fractions(canonical_units, fraction_step)
    return canonical_fluids, canonical_fractions, canonical_units


def canonicalize_individual(individual: Individual, fraction_step: float) -> Individual:
    """Seřadí látky jedince a podíly podle kanonické reprezentace."""
    fluids, fractions, _ = canonicalize_mixture(
        individual.fluids, individual.fractions, fraction_step
    )
    return Individual(
        fluids=fluids,
        fractions=fractions,
...
def random_fraction_grid(
    ...
    Používá uniformní sampling všech kladných celočíselných řešení rovnice
    x1 + x2 + ... + xN = total_units. Prakticky se náhodně vybere N-1 řezů
    mezi jednotkami a rozdíly mezi řezy jsou váhy složek.
    """
    total_units = fraction_units(fraction_step)
    if n_components > total_units:
        raise ValueError(
    ...
    cuts = np.sort(rng.choice(np.arange(1, total_units), size=n_components - 1, replace=False))
    units = np.diff(np.concatenate(([0], cuts, [total_units])))
    return units_to_fractions(units, fraction_step)
...
def make_cache_key(
    ...
    """Cache key nezávislý na pořadí složek.

    Parametr decimals zůstává v API kvůli kompatibilitě se starším voláním, ale
    pro diskrétní podíly je bezpečnější používat celočíselné jednotky.
    """
    del decimals
    canonical_fluids, _, units = canonicalize_mixture(fluids, fractions, fraction_step)
    return tuple(zip(canonical_fluids, units))
...
def evaluate_mixture(
    ...
    """Spočítá jeden konkrétní bod směsi pro cyklus s vnitřním výměníkem.

    Důležité: tato funkce sama neprochází žádnou mřížku podílů. Vždy dostane
    právě jednu kombinaci látek a právě jedno složení vytvořené GA. Zbytek
    souboru je záměrně shodný s BASIC_GA.py; unikátní část IHX varianty je
    fyzikální výpočet uvnitř této funkce.
    """
    ...
    try:
        fluids, fractions, _ = canonicalize_mixture(
            fluids, fractions, params.fraction_step
        )
        key = make_cache_key(fluids, fractions, decimals=10, fraction_step=params.fraction_step)
    except Exception as exc:
        raw_key = tuple(zip((str(fluid) for fluid in fluids), np.asarray(fractions).tolist()))
        return penalty_evaluation(raw_key, str(exc))
    ...
    try:
        ...
        P1ihx = P1
        T1ihx = T3 - params.dT_IHX
        if T1ihx <= params.T_evap:
            raise ValueError(
                f"Nefyzikalni IHX bod: T1ihx={T1ihx:.6g} K <= T_evap={params.T_evap:.6g} K"
            )
        ...
        AS.update(cp.PSmass_INPUTS, P3, s1ihx)
        h2s = AS.hmass()

        h2 = h1ihx + (h2s - h1ihx) / params.eta_comp
...
def evaluate_worker(args: Tuple[Tuple[str, ...], np.ndarray, CycleParams, int, float]) -> Evaluation:
    fluids, fractions, params, cache_decimals, fraction_step = args
    result = evaluate_mixture(fluids, fractions, params)
    return Evaluation(
        make_cache_key(fluids, fractions, cache_decimals, fraction_step),
        result.fitness,
        result.COP,
        result.VHC,
        result.W_comp,
...
def random_individual(
    ...
    return canonicalize_individual(Individual(fluids, fractions), fraction_step)


def seeded_component_individual(
    fluids: Sequence[str],
    fraction_step: float,
    rng: np.random.Generator,
) -> Individual:
    """Vytvoří jedince s pevně zadanými komponentami a náhodnými validními vahami."""
    fluids = tuple(str(fluid) for fluid in fluids)
    fractions = random_fraction_grid(len(fluids), fraction_step, rng)
    return canonicalize_individual(Individual(fluids, fractions), fraction_step)


def individual_key(
    individual: Individual, cache_decimals: int, fraction_step: float
) -> Tuple:
    """Jednoznačný klíč jedince pro cache i kontrolu duplicit."""
    return make_cache_key(
        individual.fluids, individual.fractions, cache_decimals, fraction_step
    )
...
def evaluation_for(
    ...
    return cache[individual_key(individual, cache_decimals, fraction_step)]
...
def fast_non_dominated_sort(
    ...
    for i, individual_i in enumerate(population):
        eval_i = evaluation_for(individual_i, cache, cache_decimals, fraction_step)
        for j, individual_j in enumerate(population):
            if i == j:
                continue
            eval_j = evaluation_for(individual_j, cache, cache_decimals, fraction_step)
    ...
    for objective_index in range(3):
        sorted_front = sorted(
            ...
            key=lambda idx: objective_values(
                evaluation_for(population[idx], cache, cache_decimals, fraction_step)
            )[objective_index],
        )
        ...
        min_value = objective_values(
            evaluation_for(population[first], cache, cache_decimals, fraction_step)
        )[objective_index]
        max_value = objective_values(
            evaluation_for(population[last], cache, cache_decimals, fraction_step)
        )[objective_index]
        ...
        for pos in range(1, len(sorted_front) - 1):
            previous_value = objective_values(
                evaluation_for(
                    population[sorted_front[pos - 1]], cache, cache_decimals, fraction_step
                )
            )[objective_index]
...
def assign_pareto_scores(
    ...
    fronts = fast_non_dominated_sort(population, cache, cache_decimals, fraction_step)
    for front in fronts:
        assign_crowding_distance(population, front, cache, cache_decimals, fraction_step)
    return fronts
...
def crossover(
    ...
    return canonicalize_individual(Individual(tuple(fluids), fractions), fraction_step)


def choose_fraction_mutation_steps(
    small_steps: int,
    medium_rate: float,
    large_rate: float,
    rng: np.random.Generator,
) -> int:
    """Vybere velikost mutace vah: malou, střední nebo větší explorační."""
    draw = rng.random()
    if draw < large_rate:
        return int(rng.integers(6, 13))
    if draw < large_rate + medium_rate:
        return int(rng.integers(2, 6))
    return max(1, small_steps)
...
def mutate(
    ...
    redistribute_mutation_rate: float,
    rng: np.random.Generator,
) -> Individual:
    ...
    if rng.random() < redistribute_mutation_rate:
        units = np.rint(
            random_fraction_grid(len(fluids), fraction_step, rng) / fraction_step
        ).astype(np.int64)
    else:
...
def evaluate_population(
    ...
    stats = EvalStats(requests=len(population))
    missing = {}
    seen_in_request = set()

    for individual in population:
        key = individual_key(individual, cache_decimals, fraction_step)
    ...
    stats.actual_evaluations = len(missing)
...
def compromise_score(
    ...
    objective_matrix = np.array(
        [
            objective_values(evaluation_for(candidate, cache, cache_decimals, fraction_step))
            for candidate in candidates
        ],
        dtype=np.float64,
    )
    values = np.array(
        objective_values(evaluation_for(individual, cache, cache_decimals, fraction_step)),
        dtype=np.float64,
    )
...
def select_representative(
    ...
    return max(
        candidates,
        key=lambda individual: compromise_score(
            individual, candidates, cache, cache_decimals, fraction_step
        ),
    )
...
def deduplicate_population(
    ...
    while len(unique_population) < population_size and attempts < max_attempts:
        ...
        key = individual_key(immigrant, cache_decimals, fraction_step)
        if key in seen:
            continue
        seen.add(key)
...
def clone_individual(individual: Individual) -> Individual:
    """Vytvoří kopii jedince bez sdílení numpy pole fractions."""
    return Individual(
        individual.fluids,
        individual.fractions.copy(),
        individual.fitness,
...
def unique_individuals(
    individuals: Sequence[Individual], cache_decimals: int, fraction_step: float
) -> List[Individual]:
    """Vrátí jedince bez duplicit podle kanonického klíče směsi."""
    unique: List[Individual] = []
    seen: Set[Tuple] = set()

    for individual in individuals:
        canonical = canonicalize_individual(individual, fraction_step)
        key = individual_key(canonical, cache_decimals, fraction_step)
        if key in seen:
            continue
        seen.add(key)
...
def update_global_pareto_archive(
    ...
    """Aktualizuje globální Pareto archiv a vrátí, zda se archiv změnil."""
    old_keys = {individual_key(ind, cache_decimals, fraction_step) for ind in archive}
    combined = unique_individuals(
        [clone_individual(ind) for ind in archive]
        + [clone_individual(ind) for ind in candidates],
        cache_decimals,
        fraction_step,
    )
    ...
    fronts = assign_pareto_scores(combined, cache, cache_decimals, fraction_step)
    nondominated = [clone_individual(combined[idx]) for idx in fronts[0]]

    if max_archive_size is not None and len(nondominated) > max_archive_size:
        archive_front = list(range(len(nondominated)))
        ...
        nondominated.sort(key=lambda ind: -ind.crowding_distance)
        nondominated = nondominated[:max_archive_size]

    new_keys = {individual_key(ind, cache_decimals, fraction_step) for ind in nondominated}
    return nondominated, new_keys != old_keys
...
def run_ga(
    ...
    medium_mutation_rate: float = 0.25,
    large_mutation_rate: float = 0.05,
    fluid_mutation_rate: float = 0.25,
    redistribute_mutation_rate: float = 0.05,
    random_immigrant_rate: float = 0.03,
    max_archive_size: Optional[int] = None,
    use_global_archive: bool = True,
    use_crossover: bool = True,
    initial_fluid_set: Optional[Sequence[str]] = None,
    initial_fluid_set_rate: float = 0.25,
    ...
    if initial_fluid_set is not None:
        initial_fluid_set = tuple(str(fluid) for fluid in initial_fluid_set)
        if len(initial_fluid_set) != n_components:
            raise ValueError("initial_fluid_set musi mit stejnou delku jako n_components.")
        if len(set(initial_fluid_set)) != len(initial_fluid_set):
            raise ValueError("initial_fluid_set obsahuje duplicitni latky.")
        unknown = set(initial_fluid_set) - set(available_fluids)
        if unknown:
            raise ValueError(f"initial_fluid_set obsahuje nezname latky: {sorted(unknown)}")

    seeded_count = 0
    if initial_fluid_set is not None:
        seeded_count = int(round(population_size * initial_fluid_set_rate))
        seeded_count = min(population_size, max(0, seeded_count))

    population = [
        seeded_component_individual(initial_fluid_set, fraction_step, rng)
        for _ in range(seeded_count)
    ]
    population.extend(
        random_individual(available_fluids, n_components, fraction_step, rng)
        for _ in range(population_size - seeded_count)
    )
    ...
    pareto_archive: List[Individual] = []
    stagnant_archive_generations = 0

    for generation in range(generations):
        generation_stats = evaluate_population(
            population, params, cache, cache_decimals, fraction_step, processes
        )
        total_stats.add(generation_stats)
        fronts = assign_pareto_scores(population, cache, cache_decimals, fraction_step)
        current_front = [clone_individual(population[idx]) for idx in fronts[0]]
        population.sort(key=lambda ind: (ind.pareto_rank, -ind.crowding_distance))
        if use_global_archive:
            pareto_archive, archive_changed = update_global_pareto_archive(
                pareto_archive,
                population,
                cache,
                cache_decimals,
            ...
            pareto_archive = current_front
            archive_changed = True

        generation_best = select_representative(
            pareto_archive, cache, cache_decimals, fraction_step
        )
        front_evaluations = [
            evaluation_for(individual, cache, cache_decimals, fraction_step)
        ...
        history.append(
            {
                ...
                "pareto_archive_size": float(len(pareto_archive)),
                "best_COP": float(best_cop),
                "best_VHC": float(best_vhc),
                "best_W_comp": float(best_w_comp),
                "fitness_requests": float(generation_stats.requests),
                "cache_hits": float(generation_stats.cache_hits),
                "cache_misses": float(generation_stats.cache_misses),
                "cache_hit_rate": float(generation_stats.hit_rate),
                "duplicate_requests": float(generation_stats.duplicate_requests),
                "actual_evaluations": float(generation_stats.actual_evaluations),
                "cache_size": float(len(cache)),
                "unique_mixtures": float(len(cache)),
                "total_actual_evaluations": float(total_stats.actual_evaluations),
            }
        )

        best = clone_individual(generation_best)
        stagnant_archive_generations = 0 if archive_changed else stagnant_archive_generations + 1

        if log_progress:
            logging.info(
                (
                    "Generation %d: population=%d | archive=%d | "
                    "cache hit rate=%.1f %% | requests=%d | hits=%d | misses=%d | "
                    "duplicates=%d | unique evaluations=%d | max COP %.6g | "
                    "max VHC %.6g | min W_comp %.6g"
                ),
                generation,
                len(population),
                len(pareto_archive),
                100.0 * generation_stats.hit_rate,
                generation_stats.requests,
                generation_stats.cache_hits,
                generation_stats.cache_misses,
                generation_stats.duplicate_requests,
                total_stats.actual_evaluations,
                best_cop,
                best_vhc,
                best_w_comp,
            )
        ...
        if (
            ...
            and stagnant_archive_generations >= early_stopping_rounds
        ):
            logging.info(
                "early stopping: globalni Pareto archiv se nezmenil %d generaci",
                early_stopping_rounds,
            )
            break

        elites = [clone_individual(ind) for ind in population[:elite_size]]
        ...
        while len(next_population) < population_size:
            ...
            if use_crossover:
                ...
                should_mutate = rng.random() < mutation_rate
            else:
                child = clone_individual(parent_a)
                should_mutate = True

            if should_mutate:
                mutation_steps = choose_fraction_mutation_steps(
                    fraction_mutation_steps,
                    medium_mutation_rate,
                    large_mutation_rate,
                    rng,
                )
                child = mutate(
    ...
    return best, history, cache, pareto_archive
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "pro cyklus s vnitrnim vymenikem tepla. Pocet slozek nastav "
            "pres --components."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.35,
        help="Pravdepodobnost mutace po crossoveru. V mutation-only rezimu se mutuje kazdy potomek.",
    )
    parser.add_argument(
        "--fraction-mutation-steps",
        type=int,
        default=1,
        help="Velikost male mutace: kolik dilku podilu se presune mezi slozkami.",
    )
    parser.add_argument(
        "--medium-mutation-rate",
        type=float,
        default=0.25,
        help="Pravdepodobnost stredni mutace vah, typicky presun 2 az 5 dilku.",
    )
    parser.add_argument(
        "--large-mutation-rate",
        type=float,
        default=0.05,
        help="Pravdepodobnost vetsi mutace vah, typicky presun 6 az 12 dilku.",
    )
    parser.add_argument(
        "--fluid-mutation-rate",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Pravdepodobnost vetsi redistribuce vsech vah pri mutaci.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Volitelny limit globalniho Pareto archivu. 0 znamena bez limitu.",
    )
    parser.add_argument(
        "--no-global-archive",
        action="store_true",
        help="Pouzije pouze Pareto frontu aktualni generace, podobne jako starsi verze.",
    )
    parser.add_argument(
        "--no-crossover",
        action="store_true",
        help="Vypne crossover a spusti mutation-only evolucni rezim.",
    )
    parser.add_argument(
        "--initial-fluid-set",
        default="",
        help="Volitelna carkami oddelena sada komponent pro cast pocatecni populace.",
    )
    parser.add_argument(
        "--initial-fluid-set-rate",
        type=float,
        default=0.25,
        help="Podil pocatecni populace vytvoreny ze sady --initial-fluid-set.",
    )
    parser.add_argument(
        "--early-stopping",
        type=int,
        default=20,
        help="Zastavi GA, pokud se globalni Pareto archiv tolik generaci nezmeni.",
    )
    parser.add_argument(
        "--cache-decimals",
        type=int,
    ...
    parser.add_argument("--dT-IHX", type=float, default=20.0, help="Teplotni rozdil ve vnitrnim vymeniku v K.")
...
def save_pareto_front(
    ...
    for individual in pareto_front:
        evaluation = evaluation_for(individual, cache, cache_decimals, fraction_step)
        rows.append(
            [
...
def main() -> None:
    ...
    initial_fluid_set = (
        tuple(fluid.strip() for fluid in args.initial_fluid_set.split(",") if fluid.strip())
        if args.initial_fluid_set
        else None
    )
    best, history, cache, pareto_front = run_ga(
        ...
        medium_mutation_rate=args.medium_mutation_rate,
        large_mutation_rate=args.large_mutation_rate,
        fluid_mutation_rate=args.fluid_mutation_rate,
        redistribute_mutation_rate=args.redistribute_mutation_rate,
        random_immigrant_rate=args.random_immigrant_rate,
        max_archive_size=None if args.max_archive_size <= 0 else args.max_archive_size,
        use_global_archive=not args.no_global_archive,
        use_crossover=not args.no_crossover,
        initial_fluid_set=initial_fluid_set,
        initial_fluid_set_rate=args.initial_fluid_set_rate,
    ...
    best_key = make_cache_key(
        best.fluids, best.fractions, args.cache_decimals, args.fraction_step
    )
    best_eval = cache[best_key]
    save_history(history, args.history_file)
    save_pareto_front(
        pareto_front, cache, args.cache_decimals, args.fraction_step, args.pareto_file
    )
