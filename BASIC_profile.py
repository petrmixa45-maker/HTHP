...
PROFILE_TXT = os.path.join(cesta, "BASIC_quat_profile.txt")
...
def format_profile_report(stats):
    ...
    lines = [
        "Profiling summary for BASIC_multiprocess.py",
        ...
        f"  mixture setup: {stats['main_mixture_setup']:.6f}",
        ...
        f"  property reads / worker total: {((stats['property_reads'] / worker_total) * 100) if worker_total else 0:.2f} %",
    ]
    return "\n".join(lines) + "\n"
...
if __name__ == "__main__":
    ...
    writer_process = Process(target=writer, args=(queue, os.path.join(cesta, "BASIC_quat.h5"), stats_queue))
