...
# Tento skript cte vysledky z HDF5 souboru vytvoreneho skriptem BASIC_codex.py
# a vykresli zavislost COP na VHC pro kvaternarni smesi.
#
# Bezny postup:
# 1. Spustit vypocet v BASIC_codex.py, ktery vytvori BASIC_quat.h5.
...
# kazdy bod ma 5 stavu a v kazdem stavu hodnoty (P, T, h, s, ro).
...
# Cesty jsou psane stejnym stylem jako v BASIC_codex.py.
# Pokud BASIC_quat.h5 jeste lezi primo v pracovni slozce, skript ho umi najit
    ...
    with h5py.File(h5_path, "r") as h5:
        for mixture_name in h5.keys():
            ...
            for slice_name in sorted(slices_group.keys()):
                ...
                # - s2 - s1 musi byt kladne, jinak jde o fyzikalne podezrely bod.
...
def make_plot(data, output_path, show=show, hover=hover, extreme_bins=extreme_bins):
    ...
    ax.set_title("COP kvaternárních směsí v závislosti na VHC - BASIC")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Vykresli graf VHC vs. COP z vysledku v BASIC_quat.h5."
    )
    parser.add_argument(
        "-i",
        "--input",
