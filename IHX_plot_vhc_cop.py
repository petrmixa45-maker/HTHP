...
# This script reads results from the HDF5 file created by IHX_brute.py
# and plots COP against VHC for quaternary mixtures with an internal heat exchanger.
#
# Typical workflow:
# 1. Run IHX_brute.py to create IHX_quat.h5.
# 2. Run this script:
...
# each point has 7 states, each storing (P, T, h, s, ro).
# State order in IHX_brute.py:
# 0 = state 1 before IHX, 1 = state 1ihx after IHX and before compressor,
# 2 = state 2 after compressor, 3 = state 2s, 4 = state 3,
# 5 = state 3ihx, 6 = state 4.
...
# Paths follow the same style as IHX_brute.py.
# If IHX_quat.h5 is still directly in the working directory, the script can
    ...
    with h5py.File(h5_path, "r") as h5:
        for mixture_name in h5.keys():
            ...
            for slice_name in sorted(slices_group.keys()):
                ...
                # - s2 - s1ihx must be positive; otherwise the point is suspicious.
...
def make_plot(data, output_path, show=show, hover=hover, extreme_bins=extreme_bins):
    ...
    ax.set_title("COP of Quaternary Mixtures vs. VHC - IHX")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Plot VHC vs. COP from results stored in IHX_quat.h5."
    )
    parser.add_argument(
        "-i",
        "--input",
